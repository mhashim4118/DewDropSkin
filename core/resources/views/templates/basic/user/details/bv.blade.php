...
<div class="row align-items-center">
    <h4 class="mb-2 col-lg-8">@lang('BV')</h4>
    <form action="{{ route('user.bv') }}" method="GET" class="col-lg-4 form-inline bg--white">
	  @csrf
	  <div class="row">
                        ...
                        <tbody>
                            ...
                            <tr>
                             <td data-label="@lang('S No.')" >{{ ($bonuses->total() - ($bonuses->firstItem() + $key)) + 1}}</td>
                                <td data-label="@lang('BV')">{{number_format($product->amount, 2) }} PKR </td>
...
@push('breadcrumb-plugins')
    <form action="{{ route('admin.users.bv')}}" method="GET" class="form-inline float-sm-right bg--white">
