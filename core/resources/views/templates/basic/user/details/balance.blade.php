                    ...
                    <table class="table table--light style--two">
                        <thead>
                            <tr>
                                ...
                                <th scope="col">@lang('Current Balance')</th>
                                <th scope="col">@lang('Updated Date')</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($bonuses->sortByDesc('updated_at') as $key => $product)
                            <tr>
                                <td data-label="@lang('Username')">{{ __($product->username) }} </td>
                                <td data-label="@lang('Full Name')">{{ __($product->firstname) }} {{$product->lastname}}</td>
                                <td data-label="@lang('Current Balance')">{{number_format($product->balance, 2) }} PKR </td>
...
@push('breadcrumb-plugins')
    <form action="{{ route('admin.users.balance')}}" method="GET" class="form-inline float-sm-right bg--white">
