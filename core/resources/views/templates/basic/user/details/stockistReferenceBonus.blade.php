...
<div class="row align-items-center">
    <h4 class="mb-2 col-lg-8">@lang('Stockist Reference Bonus')</h4>
    <form action="{{ route('user.stockistReferenceBonus') }}" method="GET" class="col-lg-4 form-inline bg--white">
	  @csrf
	  <div class="row">
       ...
       <div class="col-lg-12 ">
                    ...
                    <table class="transection-table-2">
                        <thead>
                            <tr>
                                ...
                                <th scope="col">@lang('Stockist Reference Bonus')</th>
                                <th scope="col">@lang('Updated Date')</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($bonuses as $key => $product)
                            <tr>
                               <td data-label="@lang('S No.')" >{{ ($bonuses->total() - ($bonuses->firstItem() + $key)) + 1}}</td>
                                <td data-label="@lang('Stockist Bonus')">{{number_format($product->amount, 2) }} PKR </td>
            ...
            <div class="card-footer py-4">
                {{ $bonuses->links('admin.partials.paginate') }}
            </div>
        </div>
    </div>
</div>
...
@push('breadcrumb-plugins')
    <form action="{{ route('admin.users.stockistBonus')}}" method="GET" class="form-inline float-sm-right bg--white">
