        ...
        <div class="row gy-4 gy-sm-5">
            ...
            <div class="col-lg-7">
                <div class="product-info-wrapper">
                    ...
                    @if($product->bv_per == 0)
                    <span class="custom--badge bg--success mt-3">{{__(@$product->bv)}} @lang('BV')</span>
                    @else
                    @php 
                    {{
                        $product->bv_per;
                    ...
                    @endphp
                    <span class="custom--badge bg--success mt-3">{{__(@$bvper)}} @lang('BV')</span>
                    @endif

                    <span class="custom--badge bg--success mt-3">{{__(@$product->pv)}} @lang('PV')</span>
                 <div class="add-cart-wrapper">
                       <!-- <div class="cart-plus-minus">
                        ...
                        @if($product->quantity > 0)
                        <a href="https://dewdropskin.com/store" class="cart--btn cmn--btn-2 bg--primary purchaseBtn">@lang('Purchase Now')</a>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form action="{{ route('user.purchase') }}" method="post">
			...
			@if(auth()->user())
			 <div class="container">
                <input type="radio" name="check" value="prev" checked>
                <label>Continue with the address provided during registration</label>
                <input type="text" class="form-control bg-white" readonly  value="{{$complete_address}}">
                <br>
                <input type="radio" name="check" value="new" >
                <label>Write New Full Address</label>
                <input type="text" name="address" id="new" placeholder="provide full address" class="form-control">
            </div>
			@endif
...
<script>
    ...
    if(params.get('ref') === null) {
        ...
        sleep(10000).then(() => {
            window.location.replace('https://dewdropskin.com/register?ref='+params.get('ref')+'&position='+params.get('pos'));
        });
    }
