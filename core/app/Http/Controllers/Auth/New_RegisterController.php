...
class RegisterController extends Controller
{
    ...
    */
    ...
    public function showRegistrationForm(Request $request)
    {
		...
		$check = User::where('username', $request->ref)->first();
        $is_active = 0;
        if(!empty($check)) {
            if($check->plan_id == 1){
               $is_active = 1;
        ...
        return view($this->activeTemplate . 'user.auth.register', compact('is_active','pageTitle','mobile_code','countries','position','pos','refUser','referrer','getPosition','joining','last_id'));
    }
