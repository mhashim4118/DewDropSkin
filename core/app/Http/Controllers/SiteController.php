...
class SiteController extends Controller
{
   ...
   $id = User::where('username', $request->ref_id)->first();
        if (empty($id)) {
            return response()->json(['is_active'=>0,'success' => false, 'msg' => "<span class='help-block'><strong class='text-danger'>Referrer username not found</strong></span>"]);
        } elseif($id->plan_id == 0) {
            return response()->json(['is_active'=>0,'success' => false, 'msg' => "<span class='help-block'><strong class='text-danger'>This user has not purchased any dsp plan yet</strong></span>
                     <input type='hidden' id='referrer_id' value='$id->id' name='referrer_id'>"]);
        } elseif($id->plan_id == 1) {
            return response()->json(['is_active'=>1,'success' => true, 'msg' => "<span class='help-block'><strong class='text-success'>Referrer username matched</strong></span>
                     <input type='hidden' id='referrer_id' value='$id->id' name='referrer_id'>"]);
    ...
    public function userPosition(Request $request)
    {
        ...
        return response()->json(['success' => true, 'msg' => "<span class='help-block'><strong class='text-success'>You are joining under $join_under->username at $position  </strong></span>", 'sponid' => $join_under->id]);
    }

    public function sponsorPosition(Request $request)
    {
        // print_r($request->all());die();

        if (!$request->sponsor) {
            return response()->json(['success' => false, 'msg' => "<span class='help-block'><strong class='text-danger'>Inter Sponsor name first</strong></span>"]);
        }
        if (!$request->position) {
            return response()->json(['success' => false, 'msg' => "<span class='help-block'><strong class='text-danger'>Select your position*</strong></span>"]);
        }
        ...
        if ($user) {
            $userid = $user['id'];
            $pos = getPosition($userid, $request->position);

            $join_under = User::find($pos['pos_id']);  
            if ($join_under) {
                $usrnm = $join_under->username;
                // print_r($join_under);//die();
                // $did = "id='sponid' sponid='".$join_under->id."'";
                $did = $join_under->id;
            }else{
                $usrnm = $join_under->username;
                // $did = "id='sponid' sponid='".$userid."'";
                $did = $userid;
            }   
        ...
        return response()->json(['success' => true, 'msg' => "<span class='help-block'><strong class='text-success' >You are joining under $usrnm at $position  </strong></span>", 'sponid' => $did]);
    }
    ...
    public function productDetails($id){
        ...
        //$product->meta_keyword = implode(',', $product->meta_keyword);
        ...
        return view($this->activeTemplate.'product_detail',compact('complete_address','pageTitle','product','relateds','seoContent'));
    }
